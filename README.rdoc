...
<b>Some Development Decisions Made</b>
1. I purposely decided to not have any nested resources. There were some potential opportunities for it, but wanted to make sure there was enough flexibilty going forward. For example, given the current requirements, a booking could have been nested under showtime. However, if we ever moved to a shopping cart concept, and a user could be multiple tickets to multiple movies, that would become an issue. A screening_room definitely could be nested under movie theater, but just something I have no refactored yet.
...
4. I was having trouble writing a custom scope (one that you pass a lambda), in order to check what day a showtime is (not the day and time). As a hack, I created a seperate column in the showtime field that just stores the day. I know this is bad, but it works for now. Something I will go back and improve.
...
7. Have not really included AJAX yet to make filtering a bit smoother. That's garnish, so didn't spend too much time focusing on it at this point.
