...
2. The ticket_offerings model creates a lot of complexity in the booking process. But, it is critical to moving the project closer to a usable product. Theaters have matinees, child tickets, senior citizen pricing, etc. This needs to be modeled in, and including in the checkout process.
3. The theater manager should have a dashboard to update information and view sales stats.
4. A list of all orders, and orders by movie is important. But some highlight stats may be helpful as well (Total Revenue, Total tickets sold, etc.)
...
7. On checkout, user should be able to view their total price. Also, inputting CC information should be seamless. Using dropdowns will make it easier on mobile as well.
8. Instead of writing custom validations for CC information, and email, I will leverage gems who have done this already. May not be perfect, but provides the most amount of protection against bogus orders, upon launch. Once payment processor is hooked in, it will be running the validations anyway.
